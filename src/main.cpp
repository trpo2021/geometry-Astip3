...
bool check(string& expression, string& type) // Первоначальная проверка
{
    ...
    type = iscircle;
    if (i != 6) return 0; // Проверка типа фигуры

    int count = 0;
    while (((expression[i]) == '(' && !isdigit(expression[i])) && i < expression.size()) 
    { // Проверяем количество скобок
        count++;
        ++i;
    }

    if ((expression[i - 1]) != '(' && expression[i - 1] != ' ') return 0;
    ...
    if (count) return 0; // Если скобок не хватает
    return 1;
}
...
bool parse(string& expression, vector<double>& nums, string& type)
{
    ...
    for (int i = 0; i < expression.size(); i++) {
        ...
        {
            num.push_back(expression[i]); // По символу добавляем наше число до запятой
        }
        if (expression[i] == ',' || expression[i] == ')' || expression[i] == ' ') 
        {
            if (num.size() > 0) 
            {
                ...
                if (i + 1 == expression.size() || expression[i] == ' ') continue; // Смотрим пробелы или конец
                return 0;
            }
        }
        if (count == 2 && expression[i] == ',') 
        { // Проверка на количество числе после запятой (у круга должно остаться одно число после последней запятой, у остальных 2
            count = 0;
        }
    }
    if ((count != 0 && type != "circle") || (count != 1 && type == "circle")) return 0; // Проверка круга или другой фигуры

    return 1;
}

bool calculate(const string& expression, const vector<double>& nums, string& type) // Производим рассчеты
{
